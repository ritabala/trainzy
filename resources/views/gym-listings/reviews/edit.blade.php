...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    <div class="max-w-2xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="mt-2 text-gray-600 dark:text-gray-400">Update your review for {{ $listing->gym->name }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg">
            <div class="px-6 py-8">
                <form action="{{ route('gym-listings.reviews.update', [$listing, $review]) }}" method="POST">
                    @csrf
                    @method('PUT')
                            ...
                            @for($i = 1; $i <= 5; $i++)
                                <label class="flex items-center">
                                    <input type="radio" name="rating" value="{{ $i }}" 
                                           class="sr-only" {{ old('rating', $review->rating) == $i ? 'checked' : '' }} required>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea id="review" name="review" rows="6" required
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder-gray-400"
                                  placeholder="Share your experience with this gym...">{{ old('review', $review->review) }}</textarea>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            @php
                                $currentTags = is_array($review->tags) ? $review->tags : json_decode($review->tags, true) ?? [];
                                $oldTags = old('tags', []);
                            @endphp
                            @foreach(\App\Models\GymReview::TAGS as $tag)
                                <label class="flex items-center">
                                    <input type="checkbox" name="tags[]" value="{{ $tag }}" 
                                           ...
                                           {{ in_array($tag, $oldTags) || in_array($tag, $currentTags) ? 'checked' : '' }}>
