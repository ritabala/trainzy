<div>
        ...
        <div class="flex flex-col sm:flex-row justify-between sm:items-center pb-7 border-b border-gray-200 dark:border-gray-700">
            ...
            <div class="w-full sm:w-64 mb-4 sm:mb-0 relative">
                ...
                <input 
                    ...
                    type="text" 
                    placeholder="{{ __('settings.currencies.search_currencies') }}" 
        ...
        <div class="overflow-x-auto bg-white rounded-lg mt-6 shadow dark:bg-gray-800">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 dark:text-white dark:bg-gray-800 dark:border-gray-700 dark:border">
                <thead class="bg-gray-100 dark:bg-gray-700">
                    <tr>
                        @foreach([
                            'name' => __('settings.currencies.name'),
                            'code' => __('settings.currencies.code'),
                            'symbol' => __('settings.currencies.symbol'),
                            'decimal_places' => __('settings.currencies.decimal_places'),
                            'decimal_point' => __('settings.currencies.decimal_point'),
                            'thousands_separator' => __('settings.currencies.thousands_separator'),
                            'preview' => __('settings.currencies.preview')
                        ] as $key => $label)
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($currencies as $currency)
                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-700 text-sm" wire:key="currency-{{ $currency->id . '-' . $loop->index }}">
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">{{ $currency->code }}</td>
                            <td class="px-4 py-3 whitespace-nowrap">{{ $currency->symbol }}</td>
                            <td class="px-4 py-3 whitespace-nowrap">{{ $currency->decimal_places }}</td>
                            <td class="px-4 py-3 whitespace-nowrap">{{ $currency->decimal_point }}</td>
                            <td class="px-4 py-3 whitespace-nowrap">{{ $currency->thousands_separator }}</td>
                            <td class="px-4 py-3 whitespace-nowrap">
                                <span class="px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">
                                    {{ $currency->symbol }}{{ number_format(1234.56, (int)$currency->decimal_places, $currency->decimal_point, $currency->thousands_separator) }}
                                </span>
                            </td>
                                ...
                                <div class="flex space-x-2">
                                    ...
                                    @if($currency->id !== global_settings()->currency_id)
                                        <button class="text-gray-600 hover:text-red-600 dark:text-gray-300 dark:hover:text-red-400 relative group" 
                                                wire:click="handleDeleteCurrency({{ $currency->id }})" 
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-3 text-center text-gray-500">
                                {{ __('settings.currencies.no_currencies_found') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    @if($showEditModal && $selectedCurrency)
        <livewire:superadmin-setting.super-admin-currencies.edit-currency :currency="$selectedCurrency" />
    @endif

    @if($showAddModal)
        <livewire:superadmin-setting.super-admin-currencies.add-currency />
    @endif
</div>
