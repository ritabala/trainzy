        ...
        <div class="flex flex-col sm:flex-row justify-between items-center pb-7 border-b border-gray-200 dark:border-gray-700">
            ...
            <div class="w-full sm:w-64 mb-4 sm:mb-0 relative">
                ...
                <input 
                    ...
                    type="text" 
                    placeholder="{{ __('body_metrics.search_body_metrics') }}" 
        ...
        <div class="overflow-x-auto bg-white rounded-lg mt-6 shadow dark:bg-gray-800">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 dark:text-white dark:bg-gray-800 dark:border-gray-700 dark:border">
                <thead class="bg-gray-100 dark:bg-gray-700">
                    <tr>
                        @foreach([
                            'name' => __('body_metrics.name'),
                            'unit' => __('body_metrics.unit'),
                            'description' => __('body_metrics.description'),
                            'display_order' => __('body_metrics.display_order'),
                ...
                <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                    @forelse($bodyMetricTypes as $bodyMetricType)
                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-700 text-sm text-gray-900 dark:text-gray-300">
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                <div class="relative group" x-data="{ isTruncated: false }" x-init="isTruncated = $el.querySelector('.truncate').scrollWidth > $el.querySelector('.truncate').clientWidth">
                                    <div class="truncate">
                                        {{ Str::limit($bodyMetricType->description, 20) ?? '-' }}
                                    </div>
                                    <template x-if="isTruncated">
                                        <div class="absolute bottom-full left-0 px-2 py-1 text-xs text-white bg-gray-900 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap z-50 mb-1">
                                            {{ Str::limit($bodyMetricType->description, 20) ?? '-' }}
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                {{ $bodyMetricType->display_order }}
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap">
                                <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $bodyMetricType->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $bodyMetricType->is_active ? __('common.active') : __('common.inactive') }}
                                </span>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex space-x-2">
                                    <a href="{{ route('settings.body_metrics.edit', $bodyMetricType) }}" 
                                    ...
                                    <button wire:click="handleDeleteBodyMetricType({{ $bodyMetricType }})"
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-3 text-center text-gray-500">
                                {{ __('body_metrics.no_body_metric_types_found') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
